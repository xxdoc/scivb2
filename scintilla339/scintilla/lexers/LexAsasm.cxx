...
static const char *const AsasmWordLists[] = {
...
struct OptionSetAsasm : public OptionSet<OptionsAsasm> {
	OptionSetAsasm() {
		DefineProperty("styling.within.preprocessor", &OptionsAsasm::stylingWithinPreprocessor,
			"For C++ code, determines whether all preprocessor code is styled in the "
			"preprocessor style (0, the default) or only from the initial # to the end "
			"of the command word(1).");

		DefineProperty("lexer.Asasm.allow.dollars", &OptionsAsasm::identifiersAllowDollars,
			"Set to 0 to disallow the '$' character in identifiers with the Asasm lexer.");

		DefineProperty("lexer.Asasm.track.preprocessor", &OptionsAsasm::trackPreprocessor,
			"Set to 1 to interpret #if/#else/#endif to grey out code that is not active.");

		DefineProperty("lexer.Asasm.update.preprocessor", &OptionsAsasm::updatePreprocessor,
			"Set to 1 to update preprocessor definitions when #define found.");

		DefineProperty("lexer.Asasm.triplequoted.strings", &OptionsAsasm::triplequotedStrings,
			"Set to 1 to enable highlighting of triple-quoted strings.");

		DefineProperty("lexer.Asasm.hashquoted.strings", &OptionsAsasm::hashquotedStrings,
			"Set to 1 to enable highlighting of hash-quoted strings.");

		DefineProperty("fold", &OptionsAsasm::fold);

		DefineProperty("fold.Asasm.syntax.based", &OptionsAsasm::foldSyntaxBased,
			"Set this property to 0 to disable syntax based folding.");

		DefineProperty("fold.comment", &OptionsAsasm::foldComment,
			"This option enables folding multi-line comments and explicit fold points when using the C++ lexer. "
			"Explicit fold points allows adding extra folding by placing a //{ comment at the start and a //} "
			"at the end of a section that should fold.");

		DefineProperty("fold.Asasm.comment.multiline", &OptionsAsasm::foldCommentMultiline,
			"Set this property to 0 to disable folding multi-line comments when fold.comment=1.");

		DefineProperty("fold.Asasm.comment.explicit", &OptionsAsasm::foldCommentExplicit,
			"Set this property to 0 to disable folding explicit fold points when fold.comment=1.");

		DefineProperty("fold.Asasm.explicit.start", &OptionsAsasm::foldExplicitStart,
			"The string to use for explicit fold start points, replacing the standard //{.");

		DefineProperty("fold.Asasm.explicit.end", &OptionsAsasm::foldExplicitEnd,
			"The string to use for explicit fold end points, replacing the standard //}.");

		DefineProperty("fold.Asasm.explicit.anywhere", &OptionsAsasm::foldExplicitAnywhere,
			"Set this property to 1 to enable explicit fold points anywhere, not just in line comments.");

		DefineProperty("fold.preprocessor", &OptionsAsasm::foldPreprocessor,
			"This option enables folding preprocessor directives when using the C++ lexer. "
			"Includes C#'s explicit #region and #endregion folding directives.");

		DefineProperty("fold.compact", &OptionsAsasm::foldCompact);

		DefineProperty("fold.at.else", &OptionsAsasm::foldAtElse,
			"This option enables C++ folding on a \"} else {\" line of an if statement.");

		DefineWordListSets(AsasmWordLists);
	}
};

static const char styleSubable[] = {SCE_C_IDENTIFIER, SCE_C_COMMENTDOCKEYWORD, 0};

class LexerAsasm : public ILexerWithSubStyles {
	bool caseSensitive;
	...
	OptionsAsasm options;
	OptionSetAsasm osAsasm;
	SparseState<std::string> rawStringTerminators;
	enum { activeFlag = 0x40 };
	enum { ssIdentifier, ssDocKeyword };
	SubStyles subStyles;
public:
	explicit LexerAsasm(bool caseSensitive_) :
	...
	const char * SCI_METHOD PropertyNames() {
		return osAsasm.PropertyNames();
	}
	int SCI_METHOD PropertyType(const char *name) {
		return osAsasm.PropertyType(name);
	}
	const char * SCI_METHOD DescribeProperty(const char *name) {
		return osAsasm.DescribeProperty(name);
	}
	int SCI_METHOD PropertySet(const char *key, const char *val);
	const char * SCI_METHOD DescribeWordListSets() {
		return osAsasm.DescribeWordListSets();
	}
	...
	static ILexer *LexerFactoryAsasm() {
		return new LexerAsasm(true);
	}
	static ILexer *LexerFactoryAsasmInsensitive() {
		return new LexerAsasm(false);
	}
...
int SCI_METHOD LexerAsasm::PropertySet(const char *key, const char *val) {
	if (osAsasm.PropertySet(&options, key, val)) {
		if (strcmp(key, "lexer.Asasm.allow.dollars") == 0) {
...
int SCI_METHOD LexerAsasm::WordListSet(int n, const char *wl) {
...
void SCI_METHOD LexerAsasm::Lex(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
...
void SCI_METHOD LexerAsasm::Fold(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
...
void LexerAsasm::EvaluateTokens(std::vector<std::string> &tokens) {
...
bool LexerAsasm::EvaluateExpression(const std::string &expr, const std::map<std::string, std::string> &preprocessorDefinitions) {
...
LexerModule lmAsasm(SCLEX_Asasm, LexerAsasm::LexerFactoryAsasm, "Asasm", AsasmWordLists);
